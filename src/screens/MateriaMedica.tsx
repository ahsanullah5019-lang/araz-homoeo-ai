...
export const MateriaMedica: React.FC<MateriaMedicaProps> = ({ onBack }) => {
  ...
  const handleRemedySelect = (remedy: RemedyInfo) => {
    setSelectedRemedy(remedy);
    if (isMobile) {
      setShowMobileDetails(true); // মোবাইলে ক্লিক করলে বিস্তারিত ভিউ চালু হবে
    }
  };

  // ওষুধের বিস্তারিত রেন্ডার করার হেলপার ফাংশন
  const renderRemedyDetails = () => (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      {/* ওষুধের শিরোনাম ও সাব-টাইটেল */}
      <div style={{ borderBottom: '2px solid #e2e8f0', paddingBottom: '12px', marginBottom: '20px' }}>
        <h2 style={{ margin: 0, color: '#1e3a8a', fontSize: '24px', fontWeight: 'bold' }}>{selectedRemedy.name}</h2>
      ...
      <div style={{ backgroundColor: '#eff6ff', padding: '14px 18px', borderRadius: '8px', marginBottom: '20px', borderLeft: '5px solid #2563eb' }}>
        <strong style={{ color: '#1e40af', fontSize: '14px', display: 'block', marginBottom: '4px' }}>📌 সংক্ষেপে ওষুধের নির্যাস:</strong>
        <span style={{ color: '#334155', fontSize: '13px', lineHeight: '1.6' }}>{selectedRemedy.summary}</span>
      </div>

      {/* প্রধান প্রদর্শক লক্ষণ (Keynotes) */}
      <div style={{ marginBottom: '24px' }}>
        <h3 style={{ color: '#0f172a', fontSize: '16px', borderBottom: '1px solid #cbd5e1', paddingBottom: '6px', marginBottom: '12px' }}>
          🔑 মূল প্রদর্শক লক্ষণসমূহ (Keynotes)
        </h3>
        <ul style={{ margin: 0, paddingLeft: '20px' }}>
          {selectedRemedy.keynotes.map((item, idx) => (
            <li key={idx} style={{ marginBottom: '8px', color: '#334155', fontSize: '13px', lineHeight: '1.5' }}>{item}</li>
          ))}
        </ul>
      </div>
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            {selectedRemedy.organAffinities.map((organ, idx) => (
              <span key={idx} style={{ backgroundColor: '#e0f2fe', color: '#0369a1', padding: '5px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>
                {organ}
              </span>
            ))}
          </div>
      ...
      {/* অঙ্গভিত্তিক লক্ষণসমূহ (মোবাইলে ১ কলাম, অন্যথায় ২ কলাম) */}
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
        {selectedRemedy.mindSymptoms && (
          <div style={{ backgroundColor: '#f8fafc', padding: '14px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            <h4 style={{ color: '#1e293b', margin: '0 0 10px 0', fontSize: '15px' }}>🧠 মানসিক লক্ষণ (Mind)</h4>
        ...
        {selectedRemedy.headAndNeck && (
          <div style={{ backgroundColor: '#f8fafc', padding: '14px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            <h4 style={{ color: '#1e293b', margin: '0 0 10px 0', fontSize: '15px' }}>🗣️ মাথা, চোখ ও গলা</h4>
            <ul style={{ margin: 0, paddingLeft: '18px' }}>
        ...
        {selectedRemedy.stomachAndAbdomen && (
          <div style={{ backgroundColor: '#f8fafc', padding: '14px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            <h4 style={{ color: '#1e293b', margin: '0 0 10px 0', fontSize: '15px' }}>🫄 পাকস্থলী ও পেটের লক্ষণ</h4>
            <ul style={{ margin: 0, paddingLeft: '18px' }}>
        ...
        {selectedRemedy.respiratory && (
          <div style={{ backgroundColor: '#f8fafc', padding: '14px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
            <h4 style={{ color: '#1e293b', margin: '0 0 10px 0', fontSize: '15px' }}>🫁 শ্বাসযন্ত্র ও কাশি</h4>
            <ul style={{ margin: 0, paddingLeft: '18px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '16px', backgroundColor: '#f8fafc', padding: '14px', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '24px' }}>
        <div>
          <h4 style={{ color: '#dc2626', margin: '0 0 8px 0', fontSize: '14px' }}>⚠️ রোগ বৃদ্ধি (Aggravation)</h4>
          <ul style={{ margin: 0, paddingLeft: '18px' }}>
        ...
        <div>
          <h4 style={{ color: '#16a34a', margin: '0 0 8px 0', fontSize: '14px' }}>☘️ রোগ উপশম (Amelioration)</h4>
          <ul style={{ margin: 0, paddingLeft: '18px' }}>
      ...
      {selectedRemedy.relationships && (
        <div style={{ backgroundColor: '#fffbebfb', padding: '14px', borderRadius: '8px', border: '1px solid #fde68a', marginBottom: '30px' }}>
          <h4 style={{ color: '#b45309', margin: '0 0 10px 0', fontSize: '14px' }}>🔗 অন্যান্য ওষুধের সাথে সম্পর্ক (Relationships)</h4>
          <div style={{ fontSize: '12px', color: '#78350f', display: 'flex', flexDirection: 'column', gap: '6px' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100%', overflow: 'hidden', backgroundColor: '#f1f5f9', fontFamily: 'sans-serif' }}>
      ...
      {/* ফিক্সড টপ হেডার (পিছিয়ে যাওয়ার বাটন সহ) */}
      <div style={{ 
        display: 'flex', 
        ...
        <div>
          <h1 style={{ margin: 0, color: '#1e293b', fontSize: isMobile ? '16px' : '20px', fontWeight: 'bold' }}>📚 মেটেরিয়া মেডিকা</h1>
          {!isMobile && <p style={{ margin: '2px 0 0', color: '#64748b', fontSize: '13px' }}>হোমিওপ্যাথিক মূল প্রদর্শন লক্ষণ, মানসিক অবস্থা ও অঙ্গভিত্তিক ক্রিয়া রেফারেন্স</p>}
        </div>
        {onBack && (
          <button
            onClick={onBack}
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        {/* কলাম ২: ডেস্কটপ ভিউর জন্য নির্বাচিত ওষুধের বিস্তারিত প্রদর্শন */}
        {!isMobile && (
          <div style={{ flex: 1, overflowY: 'auto', padding: '24px', backgroundColor: '#ffffff' }}>
            {selectedRemedy && renderRemedyDetails()}
          </div>
        )}

        {/* মোবাইলের জন্য ফুলস্ক্রিন পপআপ / মোডাল ভিউ */}
        {isMobile && showMobileDetails && selectedRemedy && (
          <div style={{
            position: 'absolute',
            top: 0,
            left: 0,
            ...
            {/* মোবাইল মোডাল ব্যাক ও ক্লোজ বার */}
            <div style={{
              display: 'flex',
              justifyContent: 'space-between',
              alignItems: 'center',
              ...
              <span style={{ fontWeight: 'bold', fontSize: '16px' }}>{selectedRemedy.name}</span>
              <button 
